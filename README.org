...
* Etymology
  ...
  The name suffix ~-buffer-local~ is borrowed from [[https://github.com/vic/color-theme-buffer-local][load-theme-buffer-local]], 
  ...
  aims to solve the same problem ~load-theme-buffer-local~ aimed to solve but
